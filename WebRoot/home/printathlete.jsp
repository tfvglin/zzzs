...
<div class="fixedwide">
	<div class="tips" id="noprint">
	    <s:if test='#request.status=="已提交高水平运动员报名申请表"'>
	      <div class="leftbtn"><a href="javascript:;" class="linkbtn btnred" id="biuuu_button">打印</a></div>
	    </s:if>
	</div>
    <div class="printtable" id="myPrintArea">
		<div class="clearfix" width="922">
		    <div class="right" class="photo">
				<img src="${pageContext.request.contextPath}/upload/${request.photo}" align="right" height="112.5" width="90"/>
			</div>		    
		    <div class="title">
		        <span class="titlename" style="text-align:center;line-height: 30px;font-size: 30px;font-weight: bold;">西安电子科技大学20${request.year}年高水平运动员申请表</span>
		        <br /><span class="titlename" style="text-align:center">（截止收表日期：${request.end}）</span>
		        <br />
		        <span class="titlename" style="text-align:center">
		                            报名号：<s:if test='#request.status=="尚未提交申请表"||#request.status=="尚未提交高水平运动员申请表"'>提交后生成显示</s:if>
		                            <s:if test='#request.status!="尚未提交申请表"&&#request.status!="尚未提交高水平运动员申请表"'>${request.number}</s:if>
		        </span>
		     </div>
		</div>
		<h2>个人基本情况</h2>		
		<br /><div class="titlename">高考报名号：${request.entranceno}</div><br />	
		<table width="100%" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td width="12.5%" align="center">${request.nation}</td>
		...
		<br />
		<span class="titlename">特别提示：高中阶段获得各种奖励，请附有关获奖证书的复印件，否则将在初审时不予认可。</span>
	</div>
	<br/>	
</div>