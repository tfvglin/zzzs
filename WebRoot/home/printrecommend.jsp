...
<div class="fixedwide">
	<div class="tips" id="noprint">
	    <s:if test='#request.status=="已提交保送生报名申请表"'>
    ...
    <div class="printtable" id="myPrintArea">
		<div class="clearfix" width="922">
		    ...
		    <div class="title">
		        <span class="titlename" style="text-align:center;line-height: 30px;font-size: 30px;font-weight: bold;">西安电子科技大学20${request.year}年保送生申请表</span>
		        <br /><span class="titlename" style="text-align:center">（截止收表日期：${request.end}）</span>
		        <br />
		        <span class="titlename" style="text-align:center">
		                            报名号：<s:if test='#request.status=="尚未提交申请表"||#request.status=="尚未提交保送生申请表"'>提交后生成显示</s:if>
		                            <s:if test='#request.status!="尚未提交申请表"&&#request.status!="尚未提交保送生申请表"'>${request.number}</s:if>
		        </span>
		     </div>
		</div>
		<h2>个人基本情况</h2>		
